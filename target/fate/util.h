...
double now();
// (target build: no libsndfile) void writewav(const std::vector<double> &samples, const char *filename, int rate);
// (target build: no libsndfile) std::vector<double> readwav(const char *filename, int &rate_out);
